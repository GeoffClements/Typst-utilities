#let ou-tma(
  ...
  set document(
    title: [#course -- #coursetitle -- #box[TMA #leading-zero(tma)]
  ...
  align(center)[
    ...
    #today.display("[day padding:none]")#super(day-suffix(today)) #today.display("[month repr:long] [year]")
  ]
...
#let question(from: none) = {
  ...
  context {
    if _q-cnt.get().first() > 1 {
      pagebreak(weak: true)
    }
    [= Question #q-cnt.display()]
  }
}
...
#let subquestion() = {
  ...
  context {
    [== #q-cnt.display((..lvls) => numbering("a", lvls.at(1)))]
  }
}
...
#let subsubquestion() = {
  ...
  context {
    [=== #q-cnt.display((..lvls) => numbering("i", lvls.at(2)))]
  }
}
